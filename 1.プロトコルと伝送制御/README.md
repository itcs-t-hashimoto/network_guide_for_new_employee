...
利点
  階層化して考えることによって、各階層を独立なものとして扱うことができる。
  変更の影響が、システム全体に波及しないため、拡張性や柔軟性に富んだシステム構築ができる。
  通信の機能分割が行われるため、各階層のプロトコルの実装が容易になり、責任も明確になる。
欠点
  階層化を進めすぎてしまうと、処理が重くなったり、各階層で似たような処理をしなければならなくなる。
